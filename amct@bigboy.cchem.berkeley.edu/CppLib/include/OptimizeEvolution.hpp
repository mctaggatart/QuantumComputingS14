...
class OptimizeEvolution : public Evolution{
  ...
  matrix<std::complex<double> > lambda_rho;
...
{
	matrix<std::complex<double> > Htemp_(dim_, dim_);
	lambda_rho = MOs::Dagger(rho_desired_); //Lagrange multiplier
	double deriv;
	...
	for(int j = num_time_-1; j >=0 ; --j){
			for(size_t k = 0; k < num_controls_; ++k){			
				...
				if(j) lambda_rho = MOs::Dagger(Unitaries_[j])*lambda_rho*Unitaries_[j]; 		 
	}
  ...
  for(size_t k = 0; k < num_controls_+2; ++k)
	...
	for(top_fidelity_=0.0, power_=0; abs(delta_fidelity) > tolerance_ && n_conseq_unimprov<20 && count_< max_iter_; count_++)
	{
		...
		if (count_%echorate==0)
		  {	std::cout << "step " << count_+1 << ", " << count_-failcount_+1 << " successes, " << failcount_ << " failures, with error " << 1-top_fidelity_ << " and average improvement " << avgimprov/echorate <<  std::endl; 			//  std::cout<<"before fidel out" << std::endl;
			fidelout << count_+1 << '\t' << 1-current_fidelity << '\t' << pow(base_a_,power_) << endl;	
			avgimprov=0;
			//	cout<<"before write controls";
			writecontrols("controls_int");
		       
			writepopulations("popul_int");	
		}
   }
	//cout<< "Delta fid55555 " << abs(delta_fidelity) << "tolerance " << tolerance_<< "conseq_unimprov " << n_conseq_unimprov << "count_" << count_ << "max_iter" << max_iter_ << endl;
...
//unedited grad phi 1, not implemented decomposition yet
...
inline double OptimizeEvolution::Phi2() const{
	...
	//removed MOs::Trace (temp1*std::conj(temp1)) to see if it fixed errors, I think that temp1 is automatically adding along the diagonals
	//removed one dim dividing /dim_
	//	cout<<" Phi2 "<< real(temp1*temp1)/dim_/dim_;
...
inline double OptimizeEvolution::GradPhi2(const size_t j, const size_t k) {
	...
	for(size_t q=0; q< dim_; ++q){
		for(size_t p=0; p<dim_; ++p){
			...
			temp2 += std::conj(Rho_[num_time_-1](p,q))*rho_desired_(p,q);
