...
int main (int argc, char const *argv[]){
	...
	cout << dim << endl;
	SweepOptimization sys(dim, num_controls, num_time, dt, "CoherentCavity_cd_pi_20ns_0.5g_0.01leakpen");
	strcat(cmd, sys.filename_);
	system(cmd);
	sys.SetNumericalParameters(fidelity=0.999999999, base_a, epsilon, tolerance, max_iter);
...
//	U_desired =  U_rot*MOs::TensorProduct(IdentCav, IdentQ);
	...
	u0.setparams(&AnalyticControl::RandomControl, 0, 0.5,  0, NULL, NULL);
	...
	u1.setparams(&AnalyticControl::RandomControl, -0.1, 0.1,  0, NULL, NULL);
