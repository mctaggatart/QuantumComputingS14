/*
Name: TestAdapEuler.cpp
Author: Jay Gambetta

Dependences: OrdinaryDiffEqs.hpp  
Brief Discription: Demonstrates the use of my ODE solver using the Euler method with an adative stepper
...
{	
	...
	//Files 
	ofstream data_out;
	string filename = "TestAdapEuler.dat";
	...
	for (size_t l=0; l < 100; l++){
		...
		for (size_t i=0; i<savepoints;i++){
			...
			error = abs(ey  - ny);
			// cout << error << endl; 
			data_out << eps_abs << " " << h << " " <<  t << " " <<  ny  << " " << ey << " " << error << endl;
		}
		datain.close();
		data_out << endl;  
	}	
