...
class Grape {
	public:
		...
		void SetNumericalParameters(const double fidelity, const double h, const double base_a, const double epsilon, const double tolerance, size_t max_iter, double qubit_freq_, double anharmonicity_);
		...
		void Setucontrol(Control& control, const size_t k);
		double Normalizeucontrol(const size_t k, double value);
		...
		matrix<std::complex<double> > GetPopulations (size_t initial_level);		
		double* Getucontrol(const size_t k);
		double GetFidelity(double (Grape::* phi)() const);
		size_t GetCount();
		...
		double top_fidelity_; //highest achieved fidelity
	...
	private:
		...
		//Control stuff (to move into Control class)
...
inline Grape::~Grape(){
	for( size_t k = 0; k < num_controls_; ++k){
		if(!externalControl) delete [] controls_[k];
		delete [] gradient_[k];
...
inline void Grape::Setucontrol(Control& control, const size_t k){
	externalControl = true;
	controls_[k] = control.u_;
	nsubpixels_[k]=control.nsubpixels_;
	
	H_controls_[k]=control.Hcontrol_;
	H_controls_tdep_[k]=control.Hcontrol_;
	controlsetflag_[k]=0;
}
...
inline size_t Grape::GetCount(){
	return count_;
}

//Returns a full matrix containing the density matrix at the final time
inline matrix<std::complex<double> > Grape::GetPopulations (size_t initial_level){
...
//	QOs::OutputMatrix(level2);
	...
	for (int k=0; k < num_time_; k++)	//Go through each time step
	{
		...
		if (k!=num_time_) propagator = rho_[k] * propagator; 
		//QOs::OutputMatrix(propagator);
	} 
...
inline double Grape::GetFidelity(double (Grape::* phi)() const){
	//the measure implemented, phi (PHI_0) is phi = trace[rho_desired* UN-1....U_0 rho_initial U_0\dg ... U_N-1\dg]
	...
	current_fidelity=(this->*phi)(); //Phi(rho_desired_,rho_[num_time_-1]);
...
void Grape::UnitaryTransfer(double (Grape::* phi)() const, double (Grape::* gradphi)(const size_t j, const size_t k) const, 
	...
	//	for(phase_=0; phase_<1.99*M_PI; phase_+=2*M_PI/nphases) 
		...
		if(delta_fidelity>0)  //the update in the controls			
		{
			...
			{	
			 ...
			 if (count_%200==1)
					std::cout << "step " << count_ << ", " << failcount_ << " failures, which has an fidelity of " << top_fidelity_ << " and a delta fidelity of " << delta_fidelity << std::endl; 			  
		}
		else {
				power_--;
				pos_count_ =0;
	...
	std::cout << "finished on step " << count_ << ", " << failcount_ << " failures, which has an fidelity of " << top_fidelity_ << " and a delta fidelity of " << delta_fidelity << std::endl;  
}
