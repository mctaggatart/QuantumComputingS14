...
int main (int argc, char const *argv[]){
	...
	cout << dim << endl;
	SweepOptimization sys(dim, num_controls, num_time, dt, "CoherentCavity_cd_pi_10ns_2g_4D");
	strcat(cmd, sys.filename_);
	system(cmd);
	sys.SetNumericalParameters(fidelity=0.999000999999, base_a, epsilon, tolerance, max_iter);
...
//	sys.gradPenalty = &OptimizeEvolution::GradLeakagePenalty;
	...
	Hdrift =  MOs::TensorProduct(HdriftCav, IdentQ) + MOs::TensorProduct(IdentCav, HdriftQ) + 2.0*Hcouple;
