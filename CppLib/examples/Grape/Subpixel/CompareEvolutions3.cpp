/*
NOT TESTED WITH EDITS TO NEW COMPILED FORM OF EVOLUTION
...
int main (int argc, char const *argv[]){
	...
	double Delta2 = Delta;
	double tolerance=std::numeric_limits<double>::min(), fidelity, base_a=1.7, epsilon=0.00005, tgate=20, dt;  //search params
	double rab = M_PI;
	...
	size_t rwa_num_time= tgate*rwasub*mult, num_controls =5;12;
...
//	system("mkdir hello");
   ...
   matrix<complex<double> >* dis;
	...
	dis[1]=(MOs::Dagger(dis[0]))*(dis[0]);
		...
		OptimizeEvolution baselinesys(dim, num_controls, rwa_num_time, rwa_dt,dis, numDis,typeDis, "OffResCoupling");

	//sys.SetTrueRhoDesired(U_desired,R,ho_des);
...
//		U_desired(dimQ,dimQ)=U_desired(dimQ+1,dimQ+1)=0;//1/sqrt(2);
		...
		baselinesys.SetNumericalParameters(fidelity=0.6, base_a, epsilon, tolerance, max_iter);	
...
baselinesys.SetRhoInitial(init);
	...
	//	baseControl[0]->SquarePulse(-det/1.2);
		...
		baseControl[num_controls-2]->Hcontrol_=baseControl[0]->Hcontrol_;
		baseControl[num_controls-3]->Hcontrol_=baseControl[1]->Hcontrol_;
		...
		baseControl[1]->drivefreq_=det/2;
		...
		baseControl[0]->framefreq_=0;
		baseControl[0]->drivefreq_=det/2;
		...
		baseControl[0]->detctrl = baseControl[num_controls-2];
			
					
	/*	baseControl[3]->pSetHcontrol = &Control::drivefreqControl;	
		baseControl[3]->pSetHgradient = &Control::drivefreqControlGradient;	
		baseControl[0]->pSetHcontrol = &Control::fmdrivenControl;	
		baseControl[0]->pSetHgradient = &Control::fmdrivenControlGradient;	
		baseControl[0]->framefreq_=0;
		baseControl[0]->drivefreq_=det/2;
		baseControl[3]->nsubpixels_=baseControl[3]->npixels_;
		baseControl[3]->ampctrl = baseControl[0];
		baseControl[0]->detctrl = baseControl[3];
		baseControl[0]->Hcontrol_ = 0.0*baseControl[0]->Hcontrol_;
	*/	
	...
	//	baseControl[0]->Hcontrol_ = baseControl[1]->Hcontrol_; // baseControl[1]->Hcontrol_;
		...
		baseControl[1]->setparams(&AnalyticControl::RandomControl, -0.0, 1.5,  0, pSetTransition, &refcontrol);
...
//		u3C.relphase_ = u1C.relphase_ = M_PI/2;
		...
		double num_time = rwa_num_time/60;
		double delta_t = rwa_dt*60;
		double sub = rwasub/60;
			
		for (size_t s=0; s<2; s++)
		  {	sys[s] = new OptimizeEvolution(dim, num_controls, num_time/mult, delta_t*mult,dis, numDis,typeDis, "OffResCoupling");
			sys[s]->SetNumericalParameters(fidelity=0.99999, base_a, epsilon, tolerance, max_iter);
sys[s]->SetRhoInitial(init);
	sys[s]->SetOppsDesired(dis);
	sys[s]->SetUDesired(U_desired);
	sys[s]->SetTrueRhoDesired(U_desired);
	sys[s]->SetOmega(50.0);
	...
	//	sys[1]->pGetGradient = 	&OptimizeEvolution::computegradient_Commute;			
		...
		//		sysControls[s][k]->pSubGradients = &AnalyticControl::setLinearSplineGradients;//setSubVoidGradients;//setPixelsGradients; //
				...
				//cout << "hcon " << sysControls[s][k]->pSetHcontrol<< endl;
		//		sysControls[s][k]->pSetHcontrol =  &Control::linearControl;
				//cout << "hconp " << &Control::linearControl<< endl;
		...
		for (size_t s=0; s<num_evol; s++)
		{
//			sysControls[s][num_controls-1]->pSetHcontrol = &Control::drivefreqControl;	
//			sysControls[s][num_controls-1]->pSetHgradient = &Control::drivefreqControlGradient;	
//			sysControls[s][1]->pSetHcontrol = &Control::fmdrivenControl;	
//			sysControls[s][1]->pSetHgradient = &Control::fmdrivenControlGradient;	
			sysControls[s][1]->framefreq_=0;
			sysControls[s][1]->drivefreq_=det/2;
...
//			sysControls[s][num_controls-2]->pSetHcontrol = &Control::drivefreqControl;	
//			sysControls[s][num_controls-2]->pSetHgradient = &Control::drivefreqControlGradient; 	
//			sysControls[s][0]->pSetHcontrol = &Control::fmdrivenControl;	//*/
//			sysControls[s][0]->pSetHgradient = &Control::fmdrivenControlGradient;	
			sysControls[s][0]->framefreq_=0;
			sysControls[s][0]->drivefreq_=det/2;
			...
			sysControls[s][0]->detctrl = sysControls[s][num_controls-2];
		}
		
//		for(int k=0; k<num_controls; k++)
//			baselinesys.Setucontrol(sysControls[1][k], k);
